...
class CliLauncher(
    ...
    ): AsyncResult[Task.Result.Unsuccessful, CliLauncher.Context] =
        CliLauncher.Context(
...
object CliLauncher {
    ...
    ): CliLauncher = new CliLauncher(name, tasks, ec)

    def apply(
        name: String,
        tasks: => Seq[Task],
        ec: ExecutionContextExecutor
    ): StdTry[CliLauncher] = indexTasks(tasks) map { ts => CliLauncher(name, ts, ec) }
    ...
    class Main(name: String, tasks: Seq[Task]) {
        ...
        def main (args: Array[String]): Unit = try {
            ...
            CliLauncher(name, tasks, ec) match {
